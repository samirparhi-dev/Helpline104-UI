...
export class SupervisorEmergencyContactsComponent implements OnInit {
  ...
  emergency_contact_related_data:Array<any>= [];
 ...
 ngDoCheck() {
  ...
  push2buffer(object) {
    const obj = {
      ...
      'emergContactName': object.name ? object.name.trim() : null,
      'location': object.location ? object.location.trim() : null,
    ...
    if (array.length !== 0) {
      this.notification_service.createEmergencyContacts(array)
        .subscribe(response => {
          ...
          this.dialogService.alert(this.currentLanguageSet.successfullyCreated, 'success');
          this.emergencyContactForm.reset();
  ...
  edit(object) {
    ...
    this.location = object.location ? object.location.trim() : null;
    this.contactNumber = object.emergContactNo;

    this.contactNoValue=null;
    this.contactNoValue=object.emergContactNo;
  ...
  update(formvalue) {
    ...
    if(this.contactNoValue === formvalue.contactNumber)
      {
        this.updateEmergencyContact(formvalue);
      }
      ...
      let flagEdit = false;
        for (let emergencyContactList of this.emergencyList) {
        if (emergencyContactList.emergContactNo === formvalue.contactNumber) {
          flagEdit = true;
          break;
        }
      }
  ...
  updateEmergencyContact(formvalue)
  {
    this.editData['emergContactName'] = formvalue.name ? formvalue.name.trim() : null;
    this.editData['designationID'] = formvalue.designation;
    this.editData['emergContactNo'] = formvalue.contactNumber;
    this.editData['location'] = formvalue.location ? formvalue.location.trim() : null;
