...
@Component({
  selector: 'app-medical-advise-report',
  templateUrl: './medical-advise-report.component.html',
  styleUrls: ['./medical-advise-report.component.css']
})
export class MedicalAdviseReportComponent implements OnInit {

  medicalAdviseForm: FormGroup;
  ...
  villages:Array<any> = [];
  searchCriterias:Array<any> = ['Disease Summary', 'SubCategory'];
  medicalAdviseList:Array<any> = [];

  ngOnInit() {
    this.assignSelectedLanguage();
    this.providerServiceMapID = { 'providerServiceMapID': this.saved_data.current_service.serviceID };
    this.createMedicalAdviseForm();
    this.today = new Date();
  ...
  createMedicalAdviseForm() {
    this.medicalAdviseForm = this.formBuilder.group({
  ...
  get startDate() {
    return this.medicalAdviseForm.controls['startDate'].value;
  }

  get endDate() {
    return this.medicalAdviseForm.controls['endDate'].value;
  }

  get district() {
    return this.medicalAdviseForm.controls['district'].value;
  }

  get subDistrict() {
    return this.medicalAdviseForm.controls['subDistrict'].value;
  }

  get village() {
    return this.medicalAdviseForm.controls['village'].value;
  }    

  get searchCriteria() {
    return this.medicalAdviseForm.controls['searchCriteria'].value;
  }
  ...
  checkEndDate() {
    ...
    if (this.endDate == null) {
      ...
      this.medicalAdviseForm.patchValue({
  ...
  searchReport() {
    let filename="Medical_Advise_Report";
    let startDate: Date = new Date(this.medicalAdviseForm.value.startDate);
    let endDate: Date = new Date(this.medicalAdviseForm.value.endDate);
    ...
    console.log("Data form value...", JSON.stringify(this.medicalAdviseForm.value));
    let reqObjForMedicalAdviseReport = {
      ...
      "providerServiceMapID": this.medicalAdviseForm.value.providerServiceMapID,
    ...
    console.log("Data form data", JSON.stringify(reqObjForMedicalAdviseReport, null, 4));

    this.reportService.getUnblockUserReport(reqObjForMedicalAdviseReport).subscribe((response) => {
      if (response) {
        saveAs(response, filename+".xlsx");
        this.alertService.alert(this.currentLanguageSet.medicalAdviseReportDownloaded);
      // console.log("Json data of response: ", JSON.stringify(response, null, 4));
      // if (response.length > 0) {
      //   this.medicalAdviseList = response;
      //   console.log("MEDICAL ADVISE RESPONSE, ", this.medicalAdviseList);
  ...
  exportToxlsx(criteria: any) {
    if (this.medicalAdviseList.length > 0) {
      let array = this.medicalAdviseList.filter(function (obj) {
        for (var key in obj) {
          if (obj[key] == null) {
            obj[key] = "";
          }
      ...
      if (array.length != 0) {
        ...
        let wb_name = "Medical Advise Report";
        const criteria_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(criteria);
        const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.medicalAdviseList, { header: (head) });
