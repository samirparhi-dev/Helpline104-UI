...
export class SupervisorAlertsNotificationsComponent implements OnInit {
  ...
  types :Array<any> = []; // array of all notification types(but here filtered with only Alert & Notification)
  roles :Array<any>= []; //array of all roles(but here filtered except Provider Admin)
  offices:Array<any> = [];

  allRoleIDs:Array<any> = [];// array of all role IDs

  minDate: Date = new Date();

  alerts_notifications:Array<any> = [];  //table array
  ...
  createAlertNotification(form_values) {
    ...
    console.log("allOfficeIDs: " + allOfficeIDs);
    ...
    if (allOfficeIDs != undefined && allOfficeIDs.length > 0) {
      ...
      // added second loop to resolve the issue of overwriting of work location id
      /*  for (let x = 0; x < allOfficeIDs.length; x++) {
         requestArray[x].workingLocationID = allOfficeIDs[x];        
       } */
    ...
    if (roleID == undefined && allOfficeIDs == undefined) {
      for (var i = 0; i < this.roles.length; i++) {
        roomArray.push(this.providerServiceMapID + "_" + this.roles[i].roleName);
      }
    }
      ...
      roomArray.push(this.providerServiceMapID + "_" + selectedRole[0].roleName);
    }
    else {
      let selectedRole = this.roles.filter((item) => {
        return item.roleID == roleID;
      ...
      for (var i = 0; i < allOfficeIDs.length; i++) {
        roomArray.push(this.providerServiceMapID + "_" + selectedRole[0].roleName + "_" + allOfficeIDs[i]);
      }
    }
