...
export class SetPasswordComponent implements OnInit {
  ...
  password:any;
	...
	updatePassword(new_pwd) {
		let transactionId=this.getUserData.transactionId;
		...
		if (new_pwd === this.confirmpwd) {
		  this.http_calls.postData(this.configService.getOpenCommonBaseURL() + 'user/setForgetPassword',
			...
			this.getUserData.transactionId=undefined
			);
		}
		else {
	...
	successCallback(response) {
		...
		this.loginService.userLogout().subscribe(res => this.handleSuccccess(res));
	}
